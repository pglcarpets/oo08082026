...
import "@focss/studio/dock.css";
import type { DockPanelDef, DockviewApiLike } from "@studio/lib/studioTypes";
import { DockFloatHeaderActions } from "@studio/components/ui/StudioDockFloatHeaderActions";
...
type DockShellProps = {
  ...
  /** Fires when every configured panel has been closed (e.g. via tab ×). */
  onEmpty?: () => void;
  theme?: "light" | "dark";
  storageKey?: string;
};
...
/** Panel ids removed from the product — close if a stale dock layout restores them. */
...
export const DockShell = ({ panels, onReadyApi, onEmpty, theme = "light", storageKey }: DockShellProps) => {
  ...
  const handleReady = (event: { api: DockviewApiLike }) => {
    ...
    const checkEmpty = () => {
      const anyOpen = panelsRef.current.some((p) => event.api.getPanel(p.id));
      if (!anyOpen) {
        if (storageKey) {
          try {
    ...
    const watchEmpty = () => {
      ...
      event.api.onDidRemovePanel?.(checkEmpty);
    };

    if (event.api.panels && event.api.panels.length > 0) {
      pruneRetiredPanels(event.api, allowedPanelIds(panelsRef.current));
      watchEmpty();
    ...
    // Register after defaults so an empty fromJSON restore does not collapse mid-init.
    watchEmpty();
    onReadyApi?.(event.api);
  };
  ...
  return (
    <div className={`dock-shell ${themeClass} ff-workspace-dock`} data-testid="dock-shell">
      <DockviewReact
        ...
        /* Host is the side panel; allow float to leave with only a small edge remaining */
        floatingGroupBounds={{
