...
### Scope
Two Supabase projects (AGENTS.md Â§4):
- **Admin** `rxzpznmxbaoxpikowmfc` â€” plans, profiles, handoffs, teams, price books, customer_queries, audit, furniture library + `catalog-assets`.
- **Products** `erpweaiypimorcunaimz` â€” catalog, configurator, descriptors, flags.

Persistence (AGENTS.md Â§5): dev=disk (`DEV_AUTH_BYPASS=1`), prod=Supabase (read-only fs). Mode-aware wrappers required (`writeFurnitureItem`, `plannerPersistenceMode`, `furnitureCatalogMode`). Never dual-write.
...
- Types regen: `db:types:admin` + `db:types` â€” verify types match schema.
- `pnpm run check:launch` (db_test_connection).
- Grep route handlers / server actions for raw disk writes (`fs.write`, `writeFileSync`, `platform/*/data/`) in prod paths â€” flag any bypassing mode-aware wrappers.
...
- RLS policy without `grant â€¦ to anon, authenticated`.
...
## Track 6 â€” Lighthouse + Performance
...
- Budgets (from oo-deep-audit-v2): LCP â‰¤2.5s, INP â‰¤200ms, CLS â‰¤0.1.
...
- Lighthouse JSON per route saved under `results/audit/lighthouse/`.
