/**
 * Floor Planner design tokens for canvas / Fabric.
 *
 * Hex pairs: keep in sync with `site/focss/planner/base/palette.css`
 * (run `pnpm run scan:tokens`). UI chrome uses semantic CSS vars
 * (`site/focss/planner/base/semantic.css`) — not duplicated here.
 *
 * This is the Planner's own copy. Studio has `studioPalette.ts`; the two are
 * intentionally independent and a change here does not propagate there.
 */
...
/** Colour palette swatches offered in the Planner colour dock. */
...
export const DEFAULT_SHEET_MM = { width_mm: 15000, height_mm: 10000 } as const;
/** Default auto-arrange gap between items (mm). */
export const DEFAULT_ARRANGE_GAP_MM = 300;
/** Default auto-arrange margin from room edge (mm). */
export const DEFAULT_ARRANGE_MARGIN_MM = 500;
